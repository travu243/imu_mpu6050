...
#define MPU6050							//The sensor used is MPU6050
#define MOTION_DRIVER_TARGET_MSP430		//The driver part uses the MSP430 driver (ported to STM32F1)
...
#define log_e  	printf	//Print information
...
//q30 format, divisor when converting long to float.
#define q30 1073741824.0f

//Gyro orientation setting
...
u8 mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO|INV_XYZ_ACCEL);//Setting up FIFO
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//Setting the Sample Rate
		if(res)return 3; 
		res=dmp_load_motion_driver_firmware();		//Load dmp firmware
		if(res)return 4; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//Set gyroscope orientation
		if(res)return 5; 
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//Setting the dmp function
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 6; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//Set the DMP output rate (maximum 200Hz)
		if(res)return 7;   
...
// After obtaining DMP processing data (note, this function requires more stacks, and local variables are a bit more)
// pitch: pitch angle accuracy: 0.1 ° range: -90.0 ° <----> +90.0 °
// roll: horizontal roller angle accuracy: 0.1 ° range: -180.0 ° <---> +180.0 °
// yaw: Treatment angle accuracy: 0.1 ° range: -180.0 ° <---> +180.0 °
// Return value: 0, normal
// Other, failure
